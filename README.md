...
3. The application should behave just as before with mock data, but now uses real data from a real backend server, with real search functionality. For example:
  ...
  * Past shows versus Upcoming shows should be distinguished in Venue and Artiste pages.
  * A user should be able to click on the venue for an upcoming show in the Artiste's page, and on that Venue's page, see the same show in the Venue Page's upcoming shows section.
